...
interface ComerciosContextType {
  ...
  getMisComercios: () => Comercio[];
  loadComercio: (id: string) => Comercio | null;
  setCurrentComercio: (id: string) => void;
  isLoading: boolean;
}
...
export const ComerciosProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const loadComerciosFromStorage = async () => {
    try {
      ...
      if (savedComercios) {
        ...
        const userComercios = parsedComercios.filter((c: Comercio) => c.id.startsWith('user_comercio'));
        setMisComercios(userComercios);
        setMisComercio(userComercios[0] || null);
      }
    } catch (error) {
  ...
  const saveComercio = async (comercioData: Omit<Comercio, 'id'>): Promise<boolean> => {
    try {
      ...
      const updatedComercios = [...comercios, newComercio];
      
      const success = await saveComerciosToStorage(updatedComercios);
      if (success) {
        setComercios(updatedComercios);
        const userComercios = updatedComercios.filter(c => c.id.startsWith('user_comercio'));
        setMisComercios(userComercios);
  ...
  const getMisComercios = (): Comercio[] => {
    return misComercios;
  };

  const setCurrentComercio = (id: string): void => {
    if (id === '' || !id) {
      setMisComercio(null);
    } else {
      const comercio = comercios.find(c => c.id === id);
      if (comercio) {
        setMisComercio(comercio);
      }
    }
  };
