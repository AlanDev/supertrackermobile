...
// Remove the conditional imports and use a simpler approach
const MapComponent = Platform.select({
  native: () => {
    const { default: MapView, Marker } = require('react-native-maps');
    return ({ negocios, onMarketSelect }: { negocios: any[], onMarketSelect: (market: any) => void }) => (
  ...
  default: () => {
    return ({ negocios }: { negocios: any[] }) => (
      <View className="h-[65vh] rounded-xl overflow-hidden bg-gray-200 items-center justify-center">
        <Text className="text-gray-600">El mapa no está disponible en la versión web</Text>
      </View>
    );
  }
})();
